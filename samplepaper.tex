...
\exp\Bigl(-\frac{1}{2}\sum_{i=1}^n\frac{\bigl(y_i-\hat y_i(x)\bigr)^2}{\sigma_i({\bf \tau})^2}\Bigr),
\end{align*}
with 
\[
...
\frac{1}{2}\sum_{i=1}^n\Bigl[\bigl(y_i-\hat y_i(x)\bigr)^2\frac{1+\sum_j\alpha_{ij}\hat p_j}{\sigma^2}
-\hat p_i\sum_j\log(1+\alpha_{ji})+\hat p_i\log\frac{\hat p_i}{\tilde p_i}\Bigr].
\]
Taking the saddle point yields the following:
\[
\hat p_i(x,{\bf y}) = \frac{1}{Z}\tilde p_i(x)\exp\Bigl(-\frac{1}{2\sigma^2}\sum_j\alpha_{ji}\bigl(y_j-\hat y_j(x)\bigr)^2+\frac{1}{2}\sum_j\log(1+\alpha_{ji})\Bigr),
\]
with $Z$ insuring the normalization~(\ref{eq:cs2}). This suggest to formulate the problem as two coupled regression problems $\hat {\bf y}(x)$ and $\hat {\bf p}(x)$ given a set of observations $\{(x^{m},{\bf y}^{m})\}$ where the loss function ${\cal L}\bigl[\hat {\bf y},\hat {\bf p}\bigr]$ is simply the empirical mean of ${\cal F}_{x,{\bf y}}\bigl[\hat {\bf p}(x)\bigr]$ over the learning set. In this scheme, $\tilde {\bf p}(x)$ appears as  a prior probability where all the prior knowledge on the cause-effect relationship has been inserted. As a consequence the optimization of $\hat p$ given the predictor $\hat y(x)$ is
formally solved explicitly. Indeed, 
$\partial {\cal L}\bigl[\hat {\bf y},\hat {\bf p}\bigr]/\partial \hat {\bf p}(x) = 0$ yields
\begin{equation}\label{eq:hatpi}
\hat p_i(x) = \frac{1}{Z}\tilde p_i(x)\exp\Bigl(-\frac{1}{2\sigma^2}\sum_j\alpha_{ji}{\mathbb E}_{data}\Bigl[\bigl(y_j-\hat y_j(x)\bigr)^2\vert x\Bigr]+\frac{1}{2}\sum_j\log(1+\alpha_{ji})\Bigr).
\end{equation}
In parallel the parameters of the model can be optimized to yield the following natural estimates:
\begin{equation}\label{eq:var}
\frac{\sigma^2}{1+\alpha_{ij}} = \frac{{\mathbb E}_{data}\bigl[\bigl(y_i-\hat y_i(x)\bigr)^2\hat p_j(x)\bigr]}{{\mathbb E}_{data}\bigl[\hat p_j(x)\bigr]},
\end{equation}
(the optimization of $\sigma$ does not yield an independent equation).
These are implicit equations, since $\hat {\bf p}(x)$ depends on the parameters $\sigma^2$ and $\alpha_{ij}$ which can be solved by alternatively updating $\hat {\bf p}$
and the parameters through equations~(\ref{eq:hatpi}) and~(\ref{eq:var}).

R1: insist somewhere that from the practical point of view, $\hat {\bf p}(x)$ is learned via the regression which given $x$ only tries to predict $\hat {bf p}(x,{\bf y})$.

R2: what would that means to optimize $\tilde p$ in the end? 

R3: Symmetry on the prior $\tilde p$.
...
with
\[
C_1[\hat {\bf p}] = \frac{1}{\sigma_0^2}{\mathbb E}\Bigl(\sum_{i=1}^n \hat p_i(x,{\bf y})\Delta y_i^2(x)\Bigr),
\]
...
\alpha &= \frac{n(1-r)}{1-(n+1)(1-r)}.
\end{align*}
Let us now compute the Hessian. Denoting
\[
C_2[\hat {\bf p}] = \frac{1}{\sigma_0^4}{\mathbb E}\Bigl[\sum_{i=1}^n \hat p_i(x,{\bf y})\Bigl(\Delta y_i^2(x)-\sum_{j=1}^n\hat p_j(x,{\bf y})\Delta y_j^2(x)\Bigr)^2\Bigr],
\]
we have
\begin{align*}
\frac{\partial^2 {\cal L}}{\partial r^2} &= -\frac{n}{2r^2}\Bigl(1+2\frac{1-r}{r}-\frac{2\alpha}{r}C_1[\hat {\bf p}]+\frac{\alpha}{2nr^2}C_2[\hat {\bf p}]\Bigr)\\[0.2cm]
\frac{\partial^2 {\cal L}}{\partial r\partial \alpha} &=\frac{1}{2r^2}\Bigl(\frac{\alpha}{2r}C_2[\hat {\bf p}]-C_1[\hat {\bf p}]\Bigr)\\[0.2cm]
...
where
\[
C_2 = \frac{1}{n\sigma_0^4}{\mathbb E}\Bigl[\sum_{i=1}^n \Bigl(\Delta y_i^2(x)-\frac{1}{n}\sum_{j=1}^n\Delta y_j^2(x)\Bigr)^2\Bigr].
\]
...
The oposite situation occurs when the trace of $H$ is negative and the determinant is positive, so to avoid that we need
\[
C_2 > 2-\frac{1}{n}.
\]
In words this means that variance of the predictors among the temporal channels has to be twice $\sigma_0^4$
to start to see something detected.

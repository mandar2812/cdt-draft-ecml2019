...
\begin{enumerate}
    \item Generate accurate predictions $y(t + \tau)$
    \item Learn the time lag structure $\rho(t + \tau, \tau, x(t))$.
\end{enumerate}
...
The predictive model consists of two estimators:
...
We express the loss function as a sum of two terms (equation \ref{eq:cdtfunc}):
...
\mathcal{L}(y(t + \tau), \hat{y}(t + \tau, x(t))) = 
\left (y(t + \tau) - \hat{y}(t + \tau, x(t)) \right)^2 
...
\mathcal{H}\left(\hat{p}(t + \tau, x(t)), \tilde{p}(t + \tau, x(t))\right) &= \hat{p}(t + \tau, x(t)) \ 
log \left( \frac{\hat{p}(t + \tau, x(t))}{\tilde{p}(t + \tau, x(t))}\right)
\end{align}
